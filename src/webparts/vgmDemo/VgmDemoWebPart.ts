...
export default class VgmDemoWebPart extends BaseClientSideWebPart<IVgmDemoWebPartProps> {
  ...
  private async renderAsync(): Promise<void> {
    ...
    const settingsModal: string = this.isHubAdmin
      ? this.modal(
          ...
          `<div class="tbxAdminNote"><strong>Configuración global</strong><span>Solo administradores de TIBOX HUB ven este panel. Los cambios se guardan en SharePoint y se aplican a todos los usuarios.</span></div><div class="tbxSettingsGrid" data-settings-grid></div><div class="tbxModalActions"><button class="tbxBtn" data-reset-settings>Restaurar valores</button><button class="tbxBtn primary" data-save-settings>Guardar cambios</button></div>`,
          false
        )
      : '';
    ...
    this.domElement.innerHTML = `
      ...
      <div class="tbxHub">
        <div class="tbxShell">
          ...
          <div class="tbxLayout">
            <main class="tbxMain">
              <section class="tbxCard" id="personas"><div class="tbxCardHead"><h2>Nuevos en Tibox</h2><button class="tbxLink" data-open="hires">Ver todos los nuevos ingresos ›</button></div><div class="tbxPeopleGrid">${this.hires.slice(0,4).map((p: Person) => this.personCard(p)).join('')}</div></section>
              <section class="tbxCard"><div class="tbxCardHead"><h2>Movimientos en Tibox</h2></div><div class="tbxMovements">${this.movement('BC','Braulio Contreras','Developer','Senior Developer','Ascenso')}${this.movement('JM','Javier Morales','Product Designer','AI & Design Specialist','Cambio de rol')}${this.movement('DH','Daniela Herrera','Analista de Proyectos','Coordinadora de Proyectos','Ascenso')}</div></section>
              <section class="tbxCard" id="cursos"><div class="tbxCardHead"><h2>Cursos del mes</h2><button class="tbxLink" data-toast="Vista completa de cursos: se conectará a la lista SharePoint.">Ver todos los cursos ›</button></div><div class="tbxCourses">${this.course('Vence pronto','warn','Obligatorio','Prevención del acoso laboral','45 min','08 septiembre · 7 días restantes')}${this.course('Activo','blue','Obligatorio','Seguridad de la información','1 h 20 min','16 septiembre · 15 días restantes')}${this.course('Nuevo','new','','Introducción a la Ley de Datos','35 min','30 septiembre · 29 días restantes')}</div><div class="tbxCourseFooter"><span class="tbxStatus" style="color:var(--green)">✓ Prevención del acoso laboral · Completado</span><span class="tbxStatus">○ Seguridad de la información · Pendiente</span><span class="tbxStatus" style="color:var(--red)">! Introducción a la Ley de Datos · Próximo a vencer</span></div></section>
              <section class="tbxCard" id="aprende"><div class="tbxCardHead"><h2>Tibox Aprende</h2><button class="tbxLink" data-toast="Aquí podremos abrir el centro completo de conocimiento.">Explorar ›</button></div><div class="tbxLearning"><div class="tbxPrompt"><label>✦ PROMPT DE LA SEMANA</label><p>“Analiza este correo, identifica las acciones que debo realizar, ordénalas por prioridad y redacta una respuesta breve y profesional.”</p><button data-copy-prompt>Copiar prompt</button></div><div class="tbxLearningList">${this.learningItem('Inteligencia Artificial','8 recursos')}${this.learningItem('Microsoft 365','4 recursos')}${this.learningItem('Seguridad','4 recursos')}${this.learningItem('Productividad','4 recursos')}</div></div></section>
              <section class="tbxCard"><div class="tbxCardHead"><h2>Tibox informa</h2></div><div class="tbxNews">${this.news('Nuevo cliente','Tibox certifica a nuevo cliente del retail','Hace 2 días')}${this.news('Proyecto terminado','Cerramos con éxito el proyecto Aurora','Hace 4 días')}${this.news('Nuevo colaborador','Le damos la bienvenida al nuevo equipo de Datos','Hace 5 días')}${this.news('Comunicado','Actualización de la política de teletrabajo','Hace 1 semana')}</div></section>
            </main>
            <aside class="tbxSide">
              <section class="tbxCard"><div class="tbxCardHead"><h3>Próximos cumpleaños</h3></div><div class="tbxBirthdayList">${this.birthday('PF','Paula Farías','Marketing · Hoy')}${this.birthday('SR','Sebastián Rivas','Infraestructura · Mañana')}${this.birthday('AL','Antonia López','Finanzas · 05 sep')}${this.birthday('RP','Rodrigo Paredes','Producto · 09 sep')}</div></section>
              <section class="tbxCard"><div class="tbxCardHead"><h3>Próximos eventos</h3></div><div class="tbxEventList">${this.event('04','SEP','Workshop Inteligencia Artificial','15:00 · Teams · 60 min')}${this.event('11','SEP','Encuentro Tibox','17:30 · Presencial · 90 min')}${this.event('18','SEP','Capacitación Excel Avanzado','10:00 · Teams · 45 min')}</div></section>
              <section class="tbxCard" id="beneficios"><div class="tbxBenefitHero"><label>♡ BENEFICIOS</label><h3>Beneficios pensados para ti</h3><p>Salud, educación y convenios comerciales disponibles para todo el equipo Tibox.</p><button data-open="benefits">Ver beneficios</button></div></section>
            </aside>
          </div>
        </div>

        ${this.modal('hiresModal','Nuevos en Tibox',`<input class="tbxModalSearch" data-hire-search placeholder="Buscar por nombre, cargo o área..." /><div class="tbxAllHires" data-all-hires>${this.hires.map((p: Person) => this.personCard(p)).join('')}</div>`,false)}
  ...
  private bindEvents(): void {
    ...
    this.domElement.querySelectorAll<HTMLElement>('[data-toast]').forEach((el: HTMLElement) => el.addEventListener('click', () => this.toast(el.getAttribute('data-toast') || 'Listo')));
    ...
    this.domElement.querySelectorAll<HTMLElement>('[data-person]').forEach((el: HTMLElement) => el.addEventListener('click', () => this.toast(`${el.getAttribute('data-person')}: el perfil completo se conectará a Microsoft 365.`)));
    ...
    globalSearch?.addEventListener('keydown', (event: KeyboardEvent) => { if (event.key === 'Enter' && globalSearch.value.trim()) this.toast(`Búsqueda: “${globalSearch.value.trim()}” (demo)`); });

    const copy: HTMLElement | null = this.domElement.querySelector('[data-copy-prompt]');
    copy?.addEventListener('click', async () => { const prompt: string = 'Analiza este correo, identifica las acciones que debo realizar, ordénalas por prioridad y redacta una respuesta breve y profesional.'; try { await navigator.clipboard.writeText(prompt); this.toast('Prompt copiado'); } catch { this.toast('No se pudo usar el portapapeles en este contexto.'); } });
  ...
  private filterHires(value: string): void {
    ...
    if (!area) return;
    const filtered: Person[] = this.hires.filter((person: Person) => `${person.name} ${person.role} ${person.area}`.toLowerCase().includes(query));
    area.innerHTML = filtered.length ? filtered.map((person: Person) => this.personCard(person)).join('') : '<p>No se encontraron personas.</p>';
  }
  ...
  private personCard(person: Person): string { return `<div class="tbxPerson" data-person="${this.escapeHtml(person.name)}"><span class="tbxAvatar">${person.initials}</span><span class="tbxPersonText"><strong>${this.escapeHtml(person.name)}</strong><span>${this.escapeHtml(person.role)} · ${this.escapeHtml(person.area)}</span><small>${this.escapeHtml(person.date)}</small></span></div>`; }
  private movement(initials: string, name: string, before: string, after: string, type: string): string { return `<div class="tbxMovement"><span class="tbxAvatar">${initials}</span><span class="tbxPersonText"><strong>${name}</strong><span>${before} → <b style="color:var(--text)">${after}</b></span></span><span class="change">↗ ${type}</span></div>`; }
  private course(state: string, stateClass: string, required: string, title: string, duration: string, deadline: string): string { return `<article class="tbxCourse"><div class="tbxCourseTop"><span class="tbxPill ${stateClass}">${state}</span>${required ? `<span class="tbxPill required">${required}</span>` : ''}</div><h3>${title}</h3><div class="tbxCourseMeta"><span>◷ ${duration}</span></div><div class="tbxCourseMeta"><span>Fecha límite: ${deadline}</span></div><button data-toast="Abrir curso: ${title}">Ir al curso</button></article>`; }
  private learningItem(title: string, count: string): string { return `<div class="tbxLearningItem"><span><strong>${title}</strong><small>${count}</small></span><span style="color:var(--support)">›</span></div>`; }
  private news(tag: string, title: string, time: string): string { return `<article class="tbxNewsItem"><label>${tag}</label><strong>${title}</strong><small>${time}</small></article>`; }
  private birthday(initials: string, name: string, meta: string): string { return `<div class="tbxBirthday"><span class="tbxAvatar">${initials}</span><span class="tbxPersonText"><strong>${name}</strong><span>${meta}</span></span><button data-birthday="${name}">Felicitar</button></div>`; }
  private event(day: string, month: string, title: string, meta: string): string { return `<div class="tbxEvent"><span class="tbxDate">${day}<small>${month}</small></span><span class="tbxPersonText"><strong>${title}</strong><span>${meta}</span></span></div>`; }
  private benefit(title: string, text: string): string { return `<article class="tbxNewsItem" style="min-height:110px"><label>BENEFICIO</label><strong>${title}</strong><small style="font-size:10px;line-height:1.5">${text}</small></article>`; }
