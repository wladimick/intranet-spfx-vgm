...
type PersonMailItem = { Title?: string; Email?: string };
type GraphDateTime = { dateTime?: string; timeZone?: string };
type GraphScheduleItem = { status?: string; subject?: string; start?: GraphDateTime; end?: GraphDateTime };
type GraphScheduleInformation = { scheduleId?: string; availabilityView?: string; scheduleItems?: GraphScheduleItem[] };
type GraphScheduleResponse = { value?: GraphScheduleInformation[] };
type RoomInterval = { start: number; end: number; busy: boolean; label: string };
...
export default class TiboxHubGraphWebPart extends TiboxHubWebPart {
  ...
  private async enhanceMicrosoft365(run: number): Promise<void> {
    ...
    if (side && !side.querySelector('[data-tbx-room-card]')) {
      side.insertBefore(this.createRoomCard(), side.firstElementChild);
    }
  ...
  private injectEnhancementStyles(hub: HTMLElement): void {
    ...
    style.textContent = `
      ...
      .tbxGraphRoomBtn:disabled{opacity:.55;cursor:not-allowed}
  ...
  private async getPeopleEmailMap(): Promise<Map<string, string>> {
    ...
    const listTitles: string[] = ['TIBOX HUB - Colaboradores', 'TIBOX HUB - Movimientos'];
  ...
  private openRoomModal(hub: HTMLElement, items: GraphScheduleItem[], dateKey: string): void {
    const previous: Element | null = hub.querySelector('[data-tbx-room-overlay]');
    ...
    overlay.innerHTML = `
      <div class="tbxGraphRoomModal" role="dialog" aria-modal="true" aria-label="Disponibilidad Sala de reuniones Tibox">
        <div class="tbxGraphRoomModalHead">
          <div><h3>Sala de reuniones Tibox</h3><p>${this.formatDateKey(dateKey)} · disponibilidad de 08:00 a 20:00</p></div>
    ...
    const close: () => void = (): void => { overlay.remove(); };
  ...
  private santiagoNow(): Date {
    const localized: string = new Date().toLocaleString('en-US', { timeZone: this.displayTimeZone });
    return new Date(localized);
  }

  private santiagoDateKey(): string {
    const date: Date = this.santiagoNow();
  ...
  private formatGraphTime(value?: string): string {
    return this.minutesLabel(this.timeMinutes(value));
  }
